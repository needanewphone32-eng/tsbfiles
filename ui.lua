...
-- ==== AUTO-APPENDED CONFIG SAVE/LOAD WRAPPER ====
-- This block was appended to provide reliable save/load functionality using the Settings tab.
-- Paste location: appended to the end of the file so it runs after your UI is created.
if not AUTO_EDITED_CONFIG_WRAPPER then
    AUTO_EDITED_CONFIG_WRAPPER = true

    -- defensive checks
    if not Window or not Tabs or not Tabs.Settings then
        pcall(function() if WindUI then WindUI:Notify({Title="Config", Content="Tabs.Settings not found", Icon="x"}) end end)
    else
        ConfigManager = Window.ConfigManager
        if not ConfigManager then
            pcall(function() WindUI:Notify({Title="Config", Content="ConfigManager missing", Icon="x"}) end)
        else
            pcall(function() if ConfigManager.Init then ConfigManager:Init(Window) end end)

            -- ensure CONFIG and STATE exist
            CONFIG = CONFIG or {}
            STATE = STATE or {}

            -- Config name input + dropdown in Settings tab
            ConfigName = ConfigName or "default"
            ConfigNameInput = Tabs.Settings:Input({ Title = "Config Name", Value = ConfigName, Callback = function(v) ConfigName = tostring(v or "default") end })

            function __GetAllConfigs()
                ok, list = pcall(function() return ConfigManager:AllConfigs() or {} end)
            ...
            ConfigsDropdown = Tabs.Settings:Dropdown({
                ...
                Values = __GetAllConfigs(),
                Value = (table.find(__GetAllConfigs(), ConfigName) and ConfigName) or nil,
                Callback = function(v)
                    ConfigName = tostring(v or "default")
                    pcall(function() ConfigNameInput:Set(ConfigName) end)
                end
            })

            Tabs.Settings:Space()

            -- helpers for Color3 serialization
            function __toSafe(tbl)
                if not tbl then return {} end
                out = {}
                for k,v in pairs(tbl) do
                    if typeof(v) == "Color3" then out[k] = {__type="Color3", r=v.R, g=v.G, b=v.B}
                    elseif type(v) == "table" then out[k] = __toSafe(v)
                    else out[k] = v end
                end
                return out
            end

            function __fromSafe(tbl)
                if not tbl then return {} end
                out = {}
                for k,v in pairs(tbl) do
                    if type(v) == "table" and v.__type == "Color3" and v.r and v.g and v.b then
                        out[k] = Color3.new(v.r, v.g, v.b)
                    elseif type(v) == "table" then out[k] = __fromSafe(v) else out[k] = v end
                end
                return out
            end

            -- helper to create config object
            function __createConfig(name)
                if not name or name == "" then return nil end
                cfg = nil
                pcall(function() cfg = ConfigManager:CreateConfig(tostring(name)) end)
                return cfg
            end

            -- helper: auto-register globals that look like WindUI elements (have Set & Get)
            function __registerAllGlobals(cfg)
                if not cfg then return end
                for k,v in pairs(_G) do
                    if type(k) == "string" and type(v) == "table" then
                        if v.Set and v.Get then
                            regname = tostring(k):gsub("%W","_")
                            pcall(function() cfg:Register(regname, v) end)
                        end
                    end
                end
            end

            -- Save button (in Tabs.Settings)
            Tabs.Settings:Button({
                ...
                Callback = function()
                    if not ConfigName or ConfigName == "" then
                        pcall(function() WindUI:Notify({Title="Config",Content="Invalid config name",Icon="x"}) end)
                        return
                    end

                    cfg = __createConfig(ConfigName)
                    if not cfg then
                        pcall(function() WindUI:Notify({Title="Config",Content="Create failed",Icon="x"}) end)
                        return
                    end

                    -- attempt to register all global UI elements so WindUI writes __elements
                    pcall(function() __registerAllGlobals(cfg) end)

                    -- write CONFIG / STATE / GUI_KEYBIND / ANIM_IDS into __custom to ensure JSON contains them
                    pcall(function()
                        cfg.Data = cfg.Data or {}
                        cfg.Data.__custom = cfg.Data.__custom or {}
                        cfg.Data.__custom.CONFIG = CONFIG or {}
                        cfg.Data.__custom.STATE = __toSafe(STATE or {})
                        if GUI_KEYBIND then
                            if typeof(GUI_KEYBIND) == "EnumItem" then cfg.Data.__custom.GUI_KEYBIND = GUI_KEYBIND.Name else cfg.Data.__custom.GUI_KEYBIND = tostring(GUI_KEYBIND) end
                        else
                            cfg.Data.__custom.GUI_KEYBIND = nil
                        end
                        cfg.Data.__custom.ANIM_IDS = ANIM_IDS or {}
                        cfg.Data.__custom.__lastSaved = os.date("%Y-%m-%d %H:%M:%S")
                    end)

                    ok, err = pcall(function() return cfg:Save() end)
                    if ok then
                        pcall(function() WindUI:Notify({Title="Config Saved",Content=tostring(ConfigName),Icon="check",Duration=2}) end)
                    else
                        pcall(function() WindUI:Notify({Title="Save Failed",Content=tostring(err),Icon="x",Duration=3}) end)
                    end

                    pcall(function() ConfigsDropdown:SetValues(__GetAllConfigs()) end)
                end
            })

            Tabs.Settings:Space()

            -- Load button (in Tabs.Settings)
            Tabs.Settings:Button({
                ...
                Callback = function()
                    if not ConfigName or ConfigName == "" then
                        pcall(function() WindUI:Notify({Title="Config",Content="No config selected",Icon="x"}) end)
                        return
                    end
                    ...
                    ok, res = pcall(function() return cfg:Load() end)
                    if not ok then
                        pcall(function() WindUI:Notify({Title="Load Failed",Content=tostring(res),Icon="x"}) end)
                        return
                    end

                    -- try restore from cfg.Data.__custom
                    pcall(function()
                        if cfg.Data and cfg.Data.__custom then
                            if cfg.Data.__custom.CONFIG then CONFIG = cfg.Data.__custom.CONFIG end
                            if cfg.Data.__custom.STATE then STATE = __fromSafe(cfg.Data.__custom.STATE) end
                            if cfg.Data.__custom.GUI_KEYBIND and type(cfg.Data.__custom.GUI_KEYBIND) == "string" and Enum.KeyCode[cfg.Data.__custom.GUI_KEYBIND] then
                                GUI_KEYBIND = Enum.KeyCode[cfg.Data.__custom.GUI_KEYBIND]
                            end
                            if cfg.Data.__custom.ANIM_IDS then ANIM_IDS = cfg.Data.__custom.ANIM_IDS end
                        end
                    end)

                    -- If WindUI restored registered elements they will be updated already.
                    -- Additionally, try to apply STATE values to globals that have :Set
                    pcall(function()
                        for k,v in pairs(STATE or {}) do
                            if type(k) == "string" and _G[k] and type(_G[k]) == "table" and _G[k].Set then
                                pcall(function() _G[k]:Set(v) end)
                            end
                        end
                    end)

                    pcall(function() WindUI:Notify({Title="Config Loaded",Content=tostring(ConfigName),Icon="refresh-cw",Duration=2}) end)
                    pcall(function() ConfigsDropdown:SetValues(__GetAllConfigs()) end)
                end
            })
            ...
            Tabs.Settings:Button({
                ...
                Callback = function()
                    pcall(function() ConfigsDropdown:SetValues(__GetAllConfigs()) end)
                    pcall(function() WindUI:Notify({Title="Config",Content="Refreshed list",Icon="check",Duration=1}) end)
                end
            })
            ...
            pcall(function()
                Window:OnClose(function()
                    ...
                    pcall(function()
                        ...
                        cfg.Data.__custom.CONFIG = CONFIG or {}
                        cfg.Data.__custom.STATE = __toSafe(STATE or {})
                        cfg.Data.__custom.GUI_KEYBIND = (typeof(GUI_KEYBIND)=="EnumItem" and GUI_KEYBIND.Name) or tostring(GUI_KEYBIND)
                        cfg.Data.__custom.ANIM_IDS = ANIM_IDS or {}
                        cfg.Data.__custom.__lastSaved = os.date("%Y-%m-%d %H:%M:%S")
                        pcall(function() __registerAllGlobals(cfg) end)
...
-- ==== END AUTO-APPENDED CONFIG SAVE/LOAD WRAPPER ====
