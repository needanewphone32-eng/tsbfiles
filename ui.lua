...
local function loadWindUI()
    ...
    for _, u in ipairs(urls) do
        local ok, res = pcall(function() return loadstring(game:HttpGetAsync(u))() end)
        if ok and type(res) == 'table' then
            return res
        end
    end
    warn('WindUI could not be loaded from remote.')
    return nil
end

local WindUI = rawget(_G, '__WINDUI_OBJ__')
if not WindUI then
    WindUI = loadWindUI()
...
-- Size presets
local __sizes = {
    { Name = 'Small',  Size = UDim2.new(0, 500, 0, 400) },
    { Name = 'Medium', Size = UDim2.new(0, 600, 0, 500) },
    { Name = 'Large',  Size = UDim2.new(0, 700, 0, 600) },
}

-- Forward decl
local function __createMain(selectedSize) end

-- Selector GUI (same theme vibe)
local function __createSelector()
    local player = Players.LocalPlayer or Players.PlayerAdded:Wait()
    local pg = player:WaitForChild('PlayerGui')

    local existing = pg:FindFirstChild('Select you gui size')
    if existing then existing:Destroy() end

    local sg = Instance.new('ScreenGui')
    sg.Name = 'Select you gui size'
    ...
    for _, info in ipairs(__sizes) do
        ...
        b.MouseEnter:Connect(function()
            TweenService:Create(b, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(82, 82, 91) }):Play()
        end)
        b.MouseLeave:Connect(function()
            TweenService:Create(b, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(63, 63, 70) }):Play()
        end)

        b.MouseButton1Click:Connect(function()
            TweenService:Create(b, TweenInfo.new(0.08), { BackgroundColor3 = Color3.fromRGB(112, 112, 122) }):Play()
            task.delay(0.09, function()
                TweenService:Create(b, TweenInfo.new(0.08), { BackgroundColor3 = Color3.fromRGB(82, 82, 91) }):Play()
            end)
            task.delay(0.18, function()
                sg:Destroy()
                __createMain(info.Size)
            end)
        end)
    end
    ...
    d.Position = UDim2.new(1, -46, 1, -46)
    d.AnchorPoint = Vector2.new(1, 1)
    d.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
    d.BorderSizePixel = 0
    d.Text = 'D' -- or use a sprite; keep simple
    d.TextColor3 = Color3.fromRGB(255, 255, 255)
    d.Font = Enum.Font.GothamBold
    d.TextSize = 18
    d.Parent = frame
    local dc = Instance.new('UICorner', d); dc.CornerRadius = UDim.new(0, 8)

    d.MouseEnter:Connect(function()
        TweenService:Create(d, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(105, 115, 245) }):Play()
    end)
...
-- Patch CreateWindow to force selected Size and capture Window
local function __patchCreateWindow(selectedSize)
    if type(WindUI) ~= 'table' then return end
    local original = WindUI.CreateWindow
    if type(original) ~= 'function' then return end
    if rawget(WindUI, '__PatchedCreateWindow') then return end

    WindUI.__PatchedCreateWindow = true
    WindUI.CreateWindow = function(self, opts)
        opts = opts or {}
        if selectedSize then opts.Size = selectedSize end
        local win = original(self, opts)
        rawset(_G, '__WINDUI_LAST_WINDOW__', win)
        return win
    end
end

-- After base runs, add topbar buttons (Keybind + Discord)
local function __postEnhance()
    local win = rawget(_G, '__WINDUI_LAST_WINDOW__')
    if not win then return end

    -- Keybind change button
    pcall(function()
        win:CreateTopbarButton('Keybind', 'command', function()
            win:Dialog({
                Title = 'Change Keybind',
                Content = 'Press any key to set as the new toggle keybind',
                Buttons = {
                    Title = 'Cancel',
                    Variant = 'Secondary'
                }
...
-- Full main creator: patch -> run base -> enhance
__createMain = function(selectedSize)
    __patchCreateWindow(selectedSize)
    -- Begin base script (modified to use existing WindUI)
